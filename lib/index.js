...
let previousValue = document.getElementById("nashatSelect").value
async function toggleNashatInput(event) {
    if (!await (window.confirm("هل أنت متأكد من إنشاء نشاط جديد؟ هناك معلومات غير محفوظة!"))) {
        return event.value = previousValue;
    };
    let value = event.value
    previousValue = value;
    if (value === "new") {
        resetData(previousValue, value);
...
async function resetData() {
    ...
    initImageSlots();
    updateCounter();
    document.getElementById("pQ").textContent = "نص السؤال"
    document.getElementsByClassName("options")[0].querySelectorAll("label").forEach(label => {
        const input = document.getElementById(label.getAttribute("for"));

        if (!input) return;
        input.type == "checkbox" ? input.checked = true : input.value = input.getAttribute("default")
    });
