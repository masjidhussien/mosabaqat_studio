...
async function loadExistingQuizzes() {
    ...
    try {
        const snapshot = await db.collection("masjidhussien").doc("mosabaqat").collection("quizzes").get();
...
async function deleteExistingQuiz() {
    ...
    if (confirmation) {
        try {
            await await db.collection("masjidhussien").doc("mosabaqat").collection("quizzes").doc(quizId).delete();
...
async function uploadToCloud() {
    ...
    try {
        ...
        await db.collection("masjidhussien").doc("mosabaqat").collection("quizzes").doc(customId).set({
            ...data,
...
function handleImg(input, key) {
    ...
    try {
        const doc = await await db.collection("masjidhussien").doc("mosabaqat").collection("quizzes").doc(customId).get();
        if (doc.exists) {
